...
% Checks if all next states has F property for state S
check(T, L, S, [], ax(F)) :-
    member([S|T2], T), !,
    checkax(T, T2, L, S, [], F).

% Checks if any next state has F property for state S
check(T, L, S, [], ex(F)) :-
    member([S|T2], T), !,
    checkex(T, T2, L, S, [], F).
...
% Checks if there exists a path where an state has property F
check(T, L, S, U, ef(F)) :-
    check(T, L, S, [], F), !;
    \+ member(S,U), !,
    member([S|T2], T),
...
% Checks if X is part of label list to S
check(_, L, S, [], X) :-
    member([S, L2], L), !,
    member(X, L2).
...
% Checks if all next state have property F
checkax(_, [[]], _, _, [], _).
    checkax(T, [[S2|T2]], L, S, [], F) :-
    check(T, L, S2, [], F),
    checkax(T, [T2], L, S, [], F).

% Checks if all reachable state have property F
checkag(_, [[]], _, _, _, _).
